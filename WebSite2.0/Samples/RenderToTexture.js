...
var renderPassFBO;
var renderPassTexture;
var renderPassDepthBuffer;
var renderPassSurfSize = 512;
...
function ContextLostHandler() {
    ...
    gl.deleteFramebuffer(renderPassFBO);
    gl.deleteTexture(renderPassTexture);
    gl.deleteRenderbuffer(renderPassDepthBuffer);
...
function InitFBO() {
    renderPassTexture = gl.createTexture();
    gl.bindTexture(gl.TEXTURE_2D, renderPassTexture);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, renderPassSurfSize, renderPassSurfSize, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);

    renderPassDepthBuffer = gl.createRenderbuffer();
    gl.bindRenderbuffer(gl.RENDERBUFFER, renderPassDepthBuffer);
    gl.renderbufferStorage(gl.RENDERBUFFER, gl.DEPTH_COMPONENT16, renderPassSurfSize, renderPassSurfSize);

    renderPassFBO = gl.createFramebuffer();
    gl.bindFramebuffer(gl.FRAMEBUFFER, renderPassFBO);
    gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, renderPassTexture, 0);
    gl.framebufferRenderbuffer(gl.FRAMEBUFFER, gl.DEPTH_ATTACHMENT, gl.RENDERBUFFER, renderPassDepthBuffer);

    var status = gl.checkFramebufferStatus(gl.FRAMEBUFFER);
    if (status != gl.FRAMEBUFFER_COMPLETE) {
        alert("invalid reflection fbo status: " + status);
    }
...
function SetupFBO(fbo, widh, height) {
...
function DrawScene() {
    ...
    SetupFBO(renderPassFBO, renderPassSurfSize, renderPassSurfSize);
    DrawBox(texture);

    var canvasSize = GetCanvasSize(gl);
    SetupFBO(null, canvasSize.width, canvasSize.height);
    DrawBox(renderPassTexture);
}
