...
function LoadCubeImage(index, cubeTexture) {
    ...
    cubeImage.src = "Media/cubeface" + index + ".jpg";
}
...
function SetUniforms(worldMatrix, viewMatrix, projMatrix, viewPos) {
    ...
    var viewPosUniform = gl.getUniformLocation(shaderProgram, "uViewPos");
    gl.uniform3fv(viewPosUniform, new Float32Array(viewPos));
}
...
function DrawScene() {
    ...
    var viewMatrix = makeLookAt(viewPos[0], viewPos[1], viewPos[2], 0, 0, 0, 0, 1, 0);
    var projMatrix = makePerspective(45, 640.0 / 480.0, 0.1, 100.0);
    SetUniforms(worldMatrix, viewMatrix, projMatrix, viewPos);
